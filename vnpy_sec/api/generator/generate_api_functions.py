...
class ApiGenerator:
    ...
    def generate_arg_dict(self, line: str):
        ...
        for arg in args:
            ...
            elif words[0] == "struct" and words[1] == "APISTRUCT":
                d[words[4].replace("*", "")] = words[2]
            elif words[0] == "struct" and words[2] != "*":
                d[words[2].replace("*", "")] = words[1]
            elif words[0] == "struct" and words[2] == "*":
                d[words[3]] = words[1]
        return d
        ...
        with open(filename, "w") as f:
            for name, d in self.callbacks.items():
                ...
                for type_ in d.values():
                    ...
                    elif type_ == "DFITCSECRspInfoField":
            ...
            for name, d in self.functions.items():
                ...
                for struct_field, struct_type in struct_fields.items():
                    if struct_type == "string":
                        ...
                        if struct_field == "requestID" or struct_field == "localOrderID"or struct_field == "sessionID":
                            line = f"\tgetLong(req, \"{struct_field}\", &myreq.{struct_field});\n"
                        else:
...
if __name__ == "__main__":
    md_generator = ApiGenerator("../include/DFITCSECMdApi.h", "DFITC", "md", "MdApi")
    md_generator.run()

    td_generator = ApiGenerator("../include/DFITCSECTraderApi.h", "DFITC", "td", "TdApi")
    td_generator.run()
